...
async function loadHeroSlider() {
    try {
        ...
        // Initialize carousel WITH auto-scroll (only for hero slider)
...
// Create custom carousel with single row for all devices (NO AUTO-SCROLL)
function createCustomCarousel(containerId, items, type) {
    ...
    // Create cards in REVERSE order (descending)
    ...
    // Initialize carousel functionality (NO AUTO-SCROLL)
    initCustomCarousel(`${containerId}-container`, `${containerId}-inner`);
}

// Initialize custom carousel functionality (NO AUTO-SCROLL)
function initCustomCarousel(containerId, innerId) {
    ...
    // NO AUTO-SCROLL FOR THESE CAROUSELS
...
// Load news content from Google Sheets (in descending order)
async function loadNews() {
    try {
        ...
        if (newsData.length === 0) {
            ...
            // Reverse the array to show last items first
            const reversedData = [...newsData].reverse();
            const newsItems = reversedData.map(row => ({
                image: row[0] || '',
...
// Load image gallery from Google Sheets (in descending order)
async function loadImageGallery() {
    try {
        ...
        if (galleryData.length === 0) {
            ...
            // Reverse the array to show last items first
            const reversedData = [...galleryData].reverse();
            const galleryItems = reversedData.map(row => ({
                image: row[0] || '',
                title: row[1] || 'Untitled',
...
// Load video gallery from Google Sheets (in descending order)
async function loadVideoGallery() {
    try {
        const videoData = await fetchSheetData('Video gallery', 'A2:C');
        ...
        if (videoData.length === 0) {
            ...
            const videoItems = reversedData.map(row => {
                ...
                return {
                    url: videoUrl,
                    title: row[1] || 'Video Content',
                    description: row[2] || 'Check out this video'
                };
